...
from phytebyte.bioactive_cmpd.clustering import Clusterer
from phytebyte.bioactive_cmpd import TargetInput
from phytebyte.modeling.input import BinaryClassifierInputFactory
...
class ModelInputLoader():
    ...
    def load(self, neg_sample_size_factor: int):
        ...
        self._negative_sampler.set_neg_sample_encoding(self._encoding)
        self._neg_cmpd_iters = self._get_neg_bioactive_cmpd_iters(
            neg_sample_size_factor)
        # Iterators turned into List below!
        self.model_input_containers = [
            BinaryClassifierInputFactory.create(
                encoding=self._encoding,
                positives=clust.get_encoded_cmpds(self._encoding),
                # [self._fingerprinter.encode(cmpd, self._encoding)
                #  for cmpd in clust.bioactive_cmpds],
                negatives=list(neg_cmpd_iter))
            for clust, neg_cmpd_iter in zip(
                self._pos_cmpd_clusters, self.neg_cmpd_iters)]
